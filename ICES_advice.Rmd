...
*If you want to keep ALL files published since 2018, simply add the year [i] in the file name to keep (for instance by adding "_", i, in the paste argument.*
...
for(j in 1:length(stocks)){
  ...
  #select the file
  stock <- stocks[j]
  stock_file <- paste("ICES_advice\\", stock, "_", i, ".pdf", sep ="")
  ...
  #extract the history of advice catch and management
  #the file should contain the "ICES.advice statement" as it is, either as column name or in first row
  if(is.null(dim(stock_list[grepl("ICES.advice" , stock_list, fixed = T)])) == F){
    stock_advice <- stock_list[grepl("ICES.advice" , stock_list, fixed = T)][[1]] %>% #extract the first df where he finds ICES.advice as is
    as.tibble() %>% mutate_all(as.character)
  } else {
    
    #if it didn't find the pattern, look for "ICES.advice" with any combination of special characters
    stock_advice <- stock_list[grepl("ICES.advice" , stock_list)][[1]] %>%
    as.tibble() %>% mutate_all(as.character)
  }

  #extract the original colname to use later on in "if" statement : says if the extract_tables function read the table correctly
  if(colnames(stock_advice[,1]) == "Year") { 
      original_colname <- "Year" #if "year as column name"
  } else if(pull(stock_advice[1,1]) == "Year") {
    original_colname <- "Year" #in case column names were not detected but column still well separated
  } else {
    original_colname <- colnames(stock_advice[,1]) #otherwise set current colname as original_colname
  }
  
  #rename the first column to Col1 so independently from its original name you can now refer to it
  stock_advice <- stock_advice %>% rename(Col1 = colnames(stock_advice[,1]))
  
  #select columns that starts with 2 for the years
  stock_advice <- stock_advice %>% filter(str_detect(Col1, "^2")) %>% tail(n=6)
  
  #need to differentiate if the extract_tables read all column or not
  if(nrow(stock_advice) == 0){
    stock_advice <- tibble() %>% mutate(Year = NA, Advice = NA, Stock = stock) #if didn't find
    
  } else if(original_colname == "Year" | stock_advice[1,1] == "Year"){ #if the table was read correctly
  ...
  } else if(ncol(stock_advice) <= 2) { #if there are one or two columns, the table was not read correctly
    
    #then extract info by splitting strg contained in 1st column, the pattern means to split between digits and letters
    stock_advice <- str_split_fixed(stock_advice$Col1, pattern = "(?<=\\D)(?=\\d)|(?<=\\d)(?=\\D)", n = 2) %>% 
      as.tibble() %>% #split the value in year and advice
