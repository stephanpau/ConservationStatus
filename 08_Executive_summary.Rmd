...
# 1. Load data
Load the files produced int he previous scripts.
```{r load_data, echo = F}
#french elasmos
...
Status_IUCN <- read_xlsx("data\\Elasmo_final_table_with_OSPAR.xlsx") #%>% #problem with reading colnames...
  filter(Region != "Brazil" & Region != "Canada" & Region != "Venezuela") %>% #remove far away countries
  mutate(Before_last_evaluation = ifelse(is.na(Before_last_status), NA, Before_last_evaluation))
...
### 4.2.3 Heatmap of all IUCN status by species and region
```{r heatmap_all}
#heatmap
