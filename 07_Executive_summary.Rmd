...
heatmap_df$Last_status <- factor(heatmap_df$Last_status ,levels = c("NE", "DD","LC", "NT", "VU", "EN", "CR", "3"))
heatmap_df$Status_FR <- factor(heatmap_df$Status_FR, levels = c("NE", "DD","LC", "NT", "VU", "EN", "CR", "Rare"))
...
ggplot(heatmap_df, aes(Region, Species)) +
  ...
  scale_fill_manual(values = IUCN_palette[1:8], na.translate = F) + 
...
ggsave("figs\\heatmap_all.png", width = 16, height  = 23, units = "cm")
```
