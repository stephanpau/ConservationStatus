...
#OSPAR Status
Status_OSPAR <- read.csv("data\\Status_OSPAR.csv", encoding = "UTF-8", check.names = F) %>% rename(Species = 1)#to have symbols
...
IUCN_DD_FR <- Status_IUCN %>% filter(Species %in% species_DD_FR) %>% filter(Region != "France") %>% na_if("NA") %>% filter(is.na(Last_status) == F) %>% mutate_all(as.character)
IUCN_DD_FR$Last_status <- factor(IUCN_DD_FR$Last_status ,levels = c("NE", "DD","LC", "NT", "VU", "EN", "CR"))
